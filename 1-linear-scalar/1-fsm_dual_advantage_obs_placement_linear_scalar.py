...
nvalx = 101
nvala = 101
x0val = np.linspace(-1,3,nvalx)
aval = np.linspace(-2,2,nvala)
...
x0guess = 1.8 #some initial guess for the initial condiiton
aguess = -0.8 #some initial guess for the model's  parameter
...
cguess = np.array([x0guess,aguess])
...
for jj in range(max_iter):
    ...
    for i in range(tind[-1]):
        ...
        #evolve forward sensitivity
        dMx , dMa = JRK4(x1,a1,dt)
        U = dMx @ U 
        V = dMx @ V + dMa
...
#%% cost function contours plot -- figure 3
plt.figure(figsize=(8,6))

[x0grid,agrid] = np.meshgrid(x0val,aval,indexing='ij')
cvalf = np.linspace(0,500,51)
cval = np.array([1,10,50,100,150,250,500])
...
#%% plots of the sensitivity of the optimal estimates of the initial condition to the first observation -- figure 4

fig, ax = plt.subplots(nrows=1,ncols=3, figsize=(20,5))
ax = ax.flat

[t1grid,t2grid] = np.meshgrid(t,t,indexing='ij')
cval = np.array([0.5,2.0,5.0,15.0,30.0])
...
manual_locations = [(0.8, 0.25), (1.1, 0.35),
                    (1.4, 0.60), (1.55,0.75),
                    (1.8, 1.00), (0.2,1.2),
                    (0.5, 1.20), (0.65,1.4),
                    (1.10, 1.50)
...
ax[1].plot(t,(y1[np.where(t==0.1)[0][0],:])**2,'C0',linewidth=3,label=r'$t_1=0.1$')
ax[1].set_xlim([0.1,2])
ax[1].set_ylim([1,5])
ax[1].set_xlabel(r'$t_2$',fontsize=18)
ax[1].set_ylabel(r'$y_1^2$',fontsize=18)

ax[2].plot(t,(y1[:,np.where(t==1.0)[0][0]])**2,'C0',linewidth=3,label=r'$t_2=1$')
...
#%% plots of the sensitivity of the optimal estimates of the model's parameter to the first observation -- figure 5

fig, ax = plt.subplots(nrows=1,ncols=3, figsize=(20,5))
ax = ax.flat

[t1grid,t2grid] = np.meshgrid(t,t,indexing='ij')
cval = np.array([0.2,0.5,2.0,5.0,10.0,30.0])
...
ax[1].plot(t,(w1[np.where(t==0.1)[0][0],:])**2,'C0',linewidth=3,label=r'$t_1=0.1$')
ax[1].set_xlim([0.1,2])
ax[1].set_ylim([0,10])
ax[1].set_xlabel(r'$t_2$',fontsize=18)
ax[1].set_ylabel(r'$w_1^2$',fontsize=18)

ax[2].plot(t,(w1[:,np.where(t==1.0)[0][0]])**2,'C0',linewidth=3,label=r'$t_2=1$')
ax[2].set_xlim([0.05,1])
ax[2].set_ylim([0,200])
ax[2].set_xlabel(r'$t_1$',fontsize=18)
...
#%% plots of the sensitivity of the optimal estimates of the initial condition to the second observation -- figure 6
...
manual_locations = [(0.2, 1.25), (0.4, 1.27),
                    (0.5, 1.30), (0.7,1.35),
                    (0.9, 1.35), (1.25, 0.8),
                    (1.0, 0.5),  (0.75,0.35),
                    (0.5, 0.1)]
...
ax[1].plot(t,(y2[np.where(t==0.1)[0][0],:])**2,'C0',linewidth=3,label=r'$t_1=0.1$')
ax[1].set_xlim([0.1,2])
ax[1].set_ylim([0,2])
ax[1].set_xlabel(r'$t_2$',fontsize=18)
ax[1].set_ylabel(r'$y_2^2$',fontsize=18)

ax[2].plot(t,(y2[:,np.where(t==1.0)[0][0]])**2,'C0',linewidth=3,label=r'$t_2=1$')
ax[2].set_xlim([0.05,1])
ax[2].set_ylim([0,200])
ax[2].set_xlabel(r'$t_1$',fontsize=18)
...
#%% plots of the sensitivity of the optimal estimates of the model's parameter to the second observation -- figure 7

fig, ax = plt.subplots(nrows=1,ncols=3, figsize=(20,5))
ax = ax.flat

[t1grid,t2grid] = np.meshgrid(t,t,indexing='ij')
cval = np.array([0.5,1.0,2.0,3.0,5.0,15])
...
manual_locations = [(0.15, 1.0), (0.3, 1.1),
                    (0.6, 1.1), (0.8,1.0), 
                    (1.0,0.85), (1.0,0.6),
                    (1.0,0.4),
                    (1.05, 0.25),(1.1,0.1)  
...
ax[1].plot(t,(w2[np.where(t==0.1)[0][0],:])**2,'C0',linewidth=3,label=r'$t_1=0.1$')
ax[1].set_xlim([0.1,2])
ax[1].set_ylim([0,100])
ax[1].set_xlabel(r'$t_2$',fontsize=18)
ax[1].set_ylabel(r'$w_2^2$',fontsize=18)

ax[2].plot(t,(w2[:,np.where(t==1.0)[0][0]])**2,'C0',linewidth=3,label=r'$t_2=1$')
ax[2].set_xlim([0.05,1])
ax[2].set_ylim([0,200])
ax[2].set_xlabel(r'$t_1$',fontsize=18)
